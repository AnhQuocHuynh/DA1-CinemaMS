...
public static class RabbitMqConfigurationExtensions
{
    public static void ConfigureCustomTopology(this IRabbitMqBusFactoryConfigurator cfg, IBusRegistrationContext context)
    {
        ...
        cfg.Message<EventEnvelope<UserProfileUpdatedPayload>>(e => e.SetEntityName("user.events"));
        cfg.Publish<EventEnvelope<UserProfileUpdatedPayload>>(e => e.ExchangeType = ExchangeType.Topic);
        cfg.Send<EventEnvelope<UserProfileUpdatedPayload>>(e => e.UseRoutingKeyFormatter(context => "user.profile.updated"));
