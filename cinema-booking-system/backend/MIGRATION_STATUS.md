...
1. Replace Catalog's `EventShowtimeClient` no-op with a showtime command/event integration.
2. Extract `booking-service` with an HTTP client for Showtime internal seat-reservation endpoints.
3. Add contract tests between Catalog, Facility, Showtime, and Booking before any frontend route switch.
4. Add data migration scripts and rollback paths for `cinema_catalog_db`, `cinema_facility_db`, and `cinema_showtime_db`.
5. Introduce API gateway routing only after service-level smoke tests pass.
