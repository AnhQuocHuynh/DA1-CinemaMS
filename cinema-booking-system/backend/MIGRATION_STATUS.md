...
- Replaced the direct Catalog -> Showtime call during event creation with `EventShowtimeClient`; the default adapter is no-op until `showtime-service` is wired.
...
- Replaced Facility's direct JPQL dependency on Showtime with `FacilityShowtimeGuard`; standalone Facility fails closed for destructive deletes until Showtime is wired.
...
|---|---|---|
| catalog-service | Extracted, buildable | Own Spring Boot app, own DB config, OpenAPI draft present. |
| facility-service | Extracted, buildable | Own Spring Boot app, own DB config, OpenAPI draft present. Delete guards fail closed until Showtime is wired. |
| showtime-service | Extracted, buildable | Own Spring Boot app, own DB/Redis config, OpenAPI draft present. Reads Catalog/Facility through HTTP clients. |
...
- Event creation no longer creates Showtime inside Catalog when running as a standalone service; this must become an inter-service command/event after `showtime-service` exists.
- Standalone Facility will reject destructive room/cinema deletes with `SHOWTIME_GUARD_UNAVAILABLE` until it can query `showtime-service`.
...
1. Wire Facility's `FacilityShowtimeGuard` to Showtime internal schedule checks instead of fail-closed.
2. Replace Catalog's `EventShowtimeClient` no-op with a showtime command/event integration.
3. Extract `booking-service` with an HTTP client for Showtime internal seat-reservation endpoints.
4. Add contract tests between Catalog, Facility, Showtime, and Booking before any frontend route switch.
5. Add data migration scripts and rollback paths for `cinema_catalog_db`, `cinema_facility_db`, and `cinema_showtime_db`.
6. Introduce API gateway routing only after service-level smoke tests pass.
