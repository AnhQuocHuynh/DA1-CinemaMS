...
export const RatingBadge: React.FC<RatingBadgeProps> = ({ type, id }) => {
  ...
  if (isLoading) {
    return <span className="inline-block w-12 h-4 bg-surface-container rounded animate-pulse" />;
  }
  ...
  if (!insight || insight.totalReviews === 0) {
    return (
      <span className="inline-flex items-center gap-1 text-sm font-semibold text-on-surface-variant">
        <Star size={13} className="text-inverse-on-surface fill-slate-300" />
        0.0
        <span className="text-xs font-normal text-inverse-on-surface">(0)</span>
      </span>
    );
  }
  ...
  return (
    <span className="inline-flex items-center gap-1 text-sm font-semibold text-on-surface-variant">
      ...
      <span className="text-xs font-normal text-on-surface-variant">({insight.totalReviews})</span>
    </span>
  );
};
